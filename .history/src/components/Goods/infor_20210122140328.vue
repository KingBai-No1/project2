...
export default {
    ...
    mounted(){
        ...
        if(this.$route.params.edit){
            this.headtitle = '编辑'
            this.axios.get("/api/goodsinfo",{params:{id:this.$route.params.edit}}).then((res)=>{
                this.infor = res.data.list
                this.changeSecond(this.infor.first_cateid)
                this.infor.specsattr = this.infor.specsattr.split(",")
                this.changeAttr(this.infor.specsid)
                this.fileList = [{url: this.infor.img }]
                this.editor.txt.html(this.infor.description)
            })
        }
    },
    ...
    methods: {
        ...
        changeAttr(m){
            console.log(m);
            this.axios.get("/api/specsinfo?id="+m).then((res)=>{
                this.specsattrarr = res.data.list[0].attrs
                // console.log(this.specsattrarr);
            })
        },
        submit(){
            this.infor.description = this.editor.txt.html()
            this.$refs['goodsinfor'].validate((val)=>{
                var ly = "/api/goodsadd"
                if(this.$route.params.edit){
                    ly ="/api/goodsedit"
                ...
                if(val){
                    ...
                    this.axios.post("/api/goodsadd",data).then((res)=>{
