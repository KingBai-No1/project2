        ...
        <el-form style="width:500px;" label-width="100px" :model="infor" ref="useradds" :rules="rules">
            <el-form-item  label="所属角色" prop="roleid"> 
                <el-select v-model="infor.roleid">
                    <el-option v-for="item in userarr" :key="item.uid" :label="item.rolename" :value="item.uid"></el-option>
                </el-select>
            </el-form-item>
...
export default {
    ...
    mounted(){
        ...
        if(this.$route.params.edit){
            this.headtitle='编辑'
            axios.get("/api/userinfo",{params:{uid:this.$route.params.edit}}).then((res)=>{
                console.log(res);
