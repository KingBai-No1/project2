...
<template>
    <div>
        ...
        <el-form label-width="100px" style="width:500px" :model="infor" :rules="rules" ref="cateadd">
...
export default {
    ...
    methods: {
        submit(){
            console.log(this.infor.status);
            this.$refs["cateadd"].validate((val)=>{
                if(val&&this.infor.status==1){
                    this.axios.post("/api/cateadd",this.infor).then((res)=>{
                        alert(res.data.msg)
